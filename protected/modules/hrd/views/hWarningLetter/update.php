<?php
/* @var $this HWarningLetterController */
/* @var $model HWarningLetter */

$this->breadcrumbs=array(
	'Hwarning Letters'=>array('index'),
	$model->id=>array('view','id'=>$model->id),
	'Update',
);

$this->menu=array(
array('label'=>'List HWarningLetter', 'url'=>array('index')),
array('label'=>'Create HWarningLetter', 'url'=>array('create')),
array('label'=>'View HWarningLetter', 'url'=>array('view', 'id'=>$model->id)),
array('label'=>'Manage HWarningLetter', 'url'=>array('admin')),
);
?>
<h1><img src="/rentalsys/img/icons/dashboard.png" alt="" /> Update Hwarning Letters</h1>
<div class="bloc">
    <div class="title">Update HWarningLetter <?php echo $model->id; ?></div> 
    <div class="content">
        <?php $this->renderPartial('_formUpdate', array('model'=>$model)); ?>
    </div>
</div>